...
LSSID=""
# LSSID=$(grep -Po "(?<=^SSID=).*" $ANCILLA_HOME/discovery.txt)

# if [ -z "$LSSID" ]
# then
#   NEWSSID=$(cat /dev/urandom | tr -dc 'a-z' | fold -w 6 | head -n 1)
#   SSID="ancilla-setup-$NEWSSID"
#   echo "SSID=$SSID" >> $HOME/discovery.txt
#   `jq '.host_apd_cfg.ssid |= "'$SSID'"' $ANCILLA_HOME/wificfg-default.json > $ANCILLA_HOME/wificfg.json`
# fi
